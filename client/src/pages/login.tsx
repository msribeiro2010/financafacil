...
export default function Login({ onLogin }: { onLogin: (user: any) => void }) {
  ...
  // Verificar se o Firebase está disponível ao carregar a página
  useEffect(() => {
    setGoogleAuthAvailable(isFirebaseAvailable());
  }, []);
      ...
      <div className="w-full max-w-5xl grid md:grid-cols-2 gap-8 items-center">
        ...
        <Card className="w-full shadow-xl border-0">
          ...
          <CardFooter className="flex flex-col space-y-4">
            ...
            <div className={`grid ${googleAuthAvailable ? 'grid-cols-2' : 'grid-cols-1'} gap-4 w-full`}>
              {googleAuthAvailable && (
                <Button 
                  variant="outline" 
                  className="space-x-2" 
